<?php
/**
 * THIS FILE IS AUTO-GENERATED BY LAZYRECORD,
 * PLEASE DO NOT MODIFY THIS FILE DIRECTLY.
 * 
 * Last Modified: April 17th at 10:03pm
 */
namespace App\Model;


use LazyRecord;
use LazyRecord\Schema\RuntimeSchema;
use LazyRecord\Schema\Relationship;

class MeasureAttributeSchemaProxy extends RuntimeSchema
{

    public static $column_names = array (
  0 => 'label',
  1 => 'identifier',
  2 => 'unit',
  3 => 'id',
);
    public static $column_hash = array (
  'label' => 1,
  'identifier' => 1,
  'unit' => 1,
  'id' => 1,
);
    public static $mixin_classes = array (
);
    public static $column_names_include_virtual = array (
  0 => 'label',
  1 => 'identifier',
  2 => 'unit',
  3 => 'id',
);

    const schema_class = 'App\\Model\\MeasureAttributeSchema';
    const collection_class = 'App\\Model\\MeasureAttributeCollection';
    const model_class = 'App\\Model\\MeasureAttribute';
    const model_name = 'MeasureAttribute';
    const model_namespace = 'App\\Model';
    const primary_key = 'id';
    const table = 'measure_attributes';
    const label = 'MeasureAttribute';

    public function __construct()
    {
        /** columns might have closure, so it can not be const */
        $this->columnData      = array( 
  'label' => array( 
      'name' => 'label',
      'attributes' => array( 
          'isa' => 'str',
          'type' => 'varchar',
          'primary' => NULL,
          'length' => 30,
        ),
    ),
  'identifier' => array( 
      'name' => 'identifier',
      'attributes' => array( 
          'isa' => 'str',
          'type' => 'varchar',
          'primary' => NULL,
          'length' => 30,
        ),
    ),
  'unit' => array( 
      'name' => 'unit',
      'attributes' => array( 
          'isa' => 'str',
          'type' => 'varchar',
          'primary' => NULL,
          'length' => 10,
        ),
    ),
  'id' => array( 
      'name' => 'id',
      'attributes' => array( 
          'isa' => 'int',
          'type' => 'int',
          'primary' => true,
          'autoIncrement' => true,
        ),
    ),
);
        $this->columnNames     = array( 
  'id',
  'label',
  'identifier',
  'unit',
);
        $this->primaryKey      = 'id';
        $this->table           = 'measure_attributes';
        $this->modelClass      = 'App\\Model\\MeasureAttribute';
        $this->collectionClass = 'App\\Model\\MeasureAttributeCollection';
        $this->label           = 'MeasureAttribute';
        $this->relations       = array( 
  'station_measure_attributes' => \LazyRecord\Schema\Relationship::__set_state(array( 
  'data' => array( 
      'type' => 1,
      'self_column' => 'id',
      'self_schema' => 'App\\Model\\MeasureAttributeSchema',
      'foreign_column' => 'attribute_id',
      'foreign_schema' => 'App\\Model\\StationMeasureAttributeSchema',
    ),
  'accessor' => 'station_measure_attributes',
  'where' => NULL,
  'orderBy' => array( 
    ),
)),
  'stations' => \LazyRecord\Schema\Relationship::__set_state(array( 
  'data' => array( 
      'type' => 4,
      'relation_junction' => 'station_measure_attributes',
      'relation_foreign' => 'station',
    ),
  'accessor' => 'stations',
  'where' => NULL,
  'orderBy' => array( 
    ),
)),
);
        $this->readSourceId    = 'default';
        $this->writeSourceId    = 'default';
        parent::__construct();
    }


    /**
     * Code block for message id parser.
     */
    private function __() {
        _('MeasureAttribute');
    }

}
